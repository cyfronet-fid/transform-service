...
def ts_to_iso_vectorized(df: DataFrame, cols: list[str]) -> None:
    ...
    for col in cols:
        ...
        is_all_numeric = non_null.apply(lambda x: isinstance(x, (int, float)) or
                                                  (isinstance(x, str) and x.isdigit())).all()

        if is_all_numeric:
            # Convert all as timestamps - much faster
            numeric_series = pd.to_numeric(series, errors='coerce')
            df[col] = pd.to_datetime(numeric_series, unit='ms', utc=True).dt.strftime('%Y-%m-%dT%H:%M:%S')
        else:
