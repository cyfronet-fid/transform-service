import hashlib
import json
from logging import getLogger

from sage.client import AggregatorClient
from sage.logging_config import configure_logging
from sage.sender import delete_all_from_solr, send_to_solr
from sage.state import get_checksum, save_checksum
from sage.transfomer import transform_raw_dataset

logger = getLogger(__name__)


DCAT_DATASET = "http://www.w3.org/ns/dcat#dataset"
ODRL_HAS_POLICY = "http://www.w3.org/ns/odrl/2/hasPolicy"
DSPACE_PARTICIPANT_ID = "https://w3id.org/dspace/2025/1/participantId"
EDC_ORIGINATOR = "edc:originator"
DCAT_SERVICE_KEY = "http://www.w3.org/ns/dcat#service"
DCAT_SERVICE_LEGACY_KEY = "dcat:service"

DCAT_DISTRIBUTION_KEY = "http://www.w3.org/ns/dcat#distribution"
DCAT_DISTRIBUTION_LEGACY_KEY = "dcat:distribution"

DCAT_ACCESS_SERVICE_KEY = "http://www.w3.org/ns/dcat#accessService"
DCAT_ACCESS_SERVICE_LEGACY_KEY = "dcat:accessService"

ODRL_POLICY_KEY = "http://www.w3.org/ns/odrl/2/hasPolicy"
ODRL_POLICY_LEGACY_KEY = "odrl:hasPolicy"


def get_jsonld_value(value):
    """
    Extract the underlying value from a JSON-LD value object.

    JSON-LD values may be represented as objects containing either
    ``@id`` (for references) or ``@value`` (for literal values).
    Plain values are returned unchanged.

    Examples:
        {"@id": "did:web:example"} -> "did:web:example"
        {"@value": "some value"} -> "some value"
        "some value" -> "some value"
    """
    if isinstance(value, dict):
        if "@id" in value:
            return value["@id"]
        if "@value" in value:
            return value["@value"]
    return value


def normalize_for_checksum(dataset):
    """
    Normalize a dataset before calculating its checksum.

    The Aggregator generates new UUIDs for some JSON-LD resources on every
    request, even when the underlying dataset has not changed. These dynamic
    identifiers must not affect the dataset checksum.

    Dynamic identifiers currently ignored:
    - ``dcat:service.@id``
    - ``dcat:distribution.dcat:accessService.@id``
    - ``odrl:hasPolicy.@id``

    Dataset ``@id`` and all other identifiers remain part of the checksum.
    """
    normalized = json.loads(json.dumps(dataset))

    for service_key in (
        DCAT_SERVICE_KEY,
        DCAT_SERVICE_LEGACY_KEY,
    ):
        service = normalized.get(service_key)

        if isinstance(service, dict):
            service.pop("@id", None)

    for distribution_key in (
        DCAT_DISTRIBUTION_KEY,
        DCAT_DISTRIBUTION_LEGACY_KEY,
    ):
        distributions = normalized.get(distribution_key)

        if isinstance(distributions, dict):
            distributions = [distributions]

        if not isinstance(distributions, list):
            continue

        for distribution in distributions:
            if not isinstance(distribution, dict):
                continue

            for access_service_key in (
                DCAT_ACCESS_SERVICE_KEY,
                DCAT_ACCESS_SERVICE_LEGACY_KEY,
            ):
                access_service = distribution.get(access_service_key)

                if isinstance(access_service, dict):
                    access_service.pop("@id", None)

    for policy_key in (
        ODRL_POLICY_KEY,
        ODRL_POLICY_LEGACY_KEY,
    ):
        policy = normalized.get(policy_key)

        if isinstance(policy, dict):
            policy.pop("@id", None)

    return normalized


def calculate_checksum(datasets):
    """
    Calculate a deterministic checksum for the dataset snapshot.

    Dataset order does not affect the checksum.
    Dynamic JSON-LD resource identifiers generated by the Aggregator
    are excluded from the checksum.
    """
    normalized = [
        normalize_for_checksum(dataset)
        for dataset in datasets
    ]

    normalized.sort(
        key=lambda dataset: dataset.get("@id", ""),
    )

    serialized = json.dumps(
        normalized,
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    )

    checksum = hashlib.sha256(
        serialized.encode("utf-8")
    ).hexdigest()

    logger.debug(
        "Calculated dataset checksum: %s",
        checksum,
    )

    return checksum


def flatten_datasets(catalogs):
    """
    Extract all raw dataset dicts from EDC catalogs.

    Supports both the old 0.14 JSON-LD keys and the new 0.17
    JSON-LD keys.

    Handles:
    - a single dataset object
    - a list of datasets
    - a missing dataset field
    - participantId represented as a JSON-LD @id object
    """
    datasets = []

    if not isinstance(catalogs, list):
        logger.error(
            "Expected list of catalog objects, got %s",
            type(catalogs).__name__,
        )
        return datasets

    logger.debug(
        "Processing %d catalog objects",
        len(catalogs),
    )

    for catalog in catalogs:
        if not isinstance(catalog, dict):
            logger.warning(
                "Skipping catalog with unexpected type: %s",
                type(catalog).__name__,
            )
            continue

        raw_datasets = catalog.get(DCAT_DATASET)

        # Backward compatibility with Federated Catalog 0.14.
        if raw_datasets is None:
            raw_datasets = catalog.get("dcat:dataset", [])

        if isinstance(raw_datasets, dict):
            logger.debug("Found a single dataset object in catalog")
            raw_datasets = [raw_datasets]

        if not isinstance(raw_datasets, list):
            logger.warning(
                "Unexpected dcat:dataset type: %s",
                type(raw_datasets).__name__,
            )
            continue

        participant_id = get_jsonld_value(catalog.get(DSPACE_PARTICIPANT_ID))

        # Backward compatibility with Federated Catalog 0.14.
        if participant_id is None:
            participant_id = catalog.get("dspace:participantId")

        originator = get_jsonld_value(catalog.get(EDC_ORIGINATOR))

        # Backward compatibility with Federated Catalog 0.14.
        if originator is None:
            originator = catalog.get("originator")

        for dataset in raw_datasets:
            if not isinstance(dataset, dict):
                logger.warning(
                    "Skipping dataset with unexpected type: %s",
                    type(dataset).__name__,
                )
                continue

            dataset["catalogue"] = participant_id or ""
            dataset["participant_id"] = participant_id
            dataset["originator"] = originator

            datasets.append(dataset)

    logger.info(
        "Flattened %d datasets from %d catalogs",
        len(datasets),
        len(catalogs),
    )

    return datasets


def main():
    logger.info("Starting metadata pipeline")

    # 1. Fetch current snapshot from Aggregator
    logger.info("Fetching catalog data from Aggregator")

    client = AggregatorClient()
    data = client.fetch_catalog()

    logger.debug(
        "Received catalog response of type: %s",
        type(data).__name__,
    )

    # 2. Flatten catalogs into datasets
    raw_datasets = flatten_datasets(data)

    logger.info(
        "Total raw datasets: %d",
        len(raw_datasets),
    )

    if not raw_datasets:
        logger.warning("No datasets found in Aggregator response")
        return

    # 3. Calculate checksum of current snapshot
    current_checksum = calculate_checksum(raw_datasets)
    previous_checksum = get_checksum()

    logger.debug(
        "Previous checksum: %s",
        previous_checksum,
    )
    logger.debug(
        "Current checksum: %s",
        current_checksum,
    )

    # 4. Skip Solr update if nothing changed
    if current_checksum == previous_checksum:
        logger.info("No changes detected in Aggregator data. " "Skipping Solr update.")
        return

    logger.info(
        "Changes detected in Aggregator data. "
        "Current checksum: %s, previous checksum: %s",
        current_checksum,
        previous_checksum,
    )

    # 5. Transform the complete snapshot BEFORE touching Solr
    logger.info(
        "Transforming %d datasets",
        len(raw_datasets),
    )

    transformed = []

    for dataset in raw_datasets:
        transformed_dataset = transform_raw_dataset(dataset)

        if transformed_dataset is not None:
            transformed.append(transformed_dataset)

    logger.info(
        "Successfully transformed %d/%d datasets",
        len(transformed),
        len(raw_datasets),
    )

    if not transformed:
        logger.error(
            "No datasets were successfully transformed. " "Keeping existing Solr data."
        )
        return

    # 6. Only now modify Solr
    logger.info(
        "Replacing existing Solr snapshot with %d datasets",
        len(transformed),
    )

    deleted = delete_all_from_solr()

    if not deleted:
        logger.error("Failed to clear Solr collection. " "Aborting pipeline.")
        return

    # 7. Index the new snapshot
    indexed = send_to_solr(transformed)

    if not indexed:
        logger.error(
            "Failed to index the new Solr snapshot. " "Checksum will NOT be updated."
        )
        return

    # 8. Only save checksum after successful indexing
    save_checksum(current_checksum)

    logger.info(
        "Metadata pipeline finished successfully. "
        "Indexed %d datasets and saved checksum %s",
        len(transformed),
        current_checksum,
    )


if __name__ == "__main__":
    configure_logging()
    main()
